...
MAX_ROUTING_ATTEMPTS = 3 # NOVO: Limite para o Router tentar diferentes modelos para a mesma tarefa
...
# Modelos (Mantidos conforme v9.4)
...
class Router:
    """NOVO AGENTE: Decide qual modelo usar para uma tarefa."""
    def __init__(self):
        self.meta_model_name = GEMINI_TEXT_MODEL_NAME # Usa o modelo de texto padrão para tomar decisões
        self.known_models = [GEMINI_TEXT_MODEL_NAME, GEMINI_IMAGE_GENERATION_MODEL_NAME]
    ...
    def parse_suggestion(self, response_text):
        """Tenta extrair uma sugestão de modelo de uma resposta de falha."""
        if not isinstance(response_text, str):
            return None
        match = re.search(r"O modelo ([\w\d/.-]+) deve ser usado", response_text)
        if match:
    ...
    def route_task(self, task_description):
        ...
        prompt = f"""
Você é um roteador de tarefas especialista. Dada a tarefa a seguir, escolha o modelo mais apropriado da lista de modelos disponíveis.
Responda APENAS com o nome exato do modelo.
...
Melhor Modelo:
"""
        chosen_model = call_gemini_api_with_retry(
            [prompt], agent_display_name, self.meta_model_name, generation_config_text
        )
        ...
        if chosen_model and chosen_model.strip() in self.known_models:
            ...
            return GEMINI_TEXT_MODEL_NAME # Fallback para o modelo de texto padrão
...
class Worker:
    ...
    def execute_sub_task(self, sub_task_description, context_text_part, uploaded_file_objects, model_to_use):
        agent_display_name = "Worker"
        print_agent_message(agent_display_name, f"Executando '{sub_task_description[:80]}...' com o modelo '{model_to_use}'")

        prompt_text_for_worker = rf"""
Você é um Agente Executor usando o modelo '{model_to_use}'. Sua tarefa atual é: "{sub_task_description}"
Contexto (resultados anteriores, objetivo original, arquivos):
{context_text_part}

Execute a tarefa.
- Se a tarefa for para criar uma descrição de imagem, seu resultado DEVE ser APENAS essa descrição.
- Se a tarefa for para gerar código, forneça o CONTEÚDO COMPLETO do arquivo com o nome indicado (ex: "Arquivo: nome.ext" ou ```python nome.py ... ```).
...
- Se identificar NOVAS sub-tarefas cruciais, liste-as em 'NOVAS_TAREFAS_SUGERIDAS:' como um array JSON de strings. Se não, omita esta seção.

Resultado da Tarefa:
"""
        prompt_parts = [prompt_text_for_worker] + uploaded_file_objects
        response_text = call_gemini_api_with_retry(
            prompt_parts, agent_display_name, model_to_use, generation_config_text
        )

        if response_text is None: return None, []
        # ... (resto da lógica de parse da resposta permanece o mesmo da v9.4) ...
...
class TaskManager:
    def __init__(self):
        ...
        self.router = Router() # NOVO: Instancia o Router
        self.task_list = []
    ...
    def run_workflow(self, initial_goal, uploaded_file_objects, uploaded_files_metadata):
        ...
        while True:
            ...
            while current_task_index < len(self.task_list):
                ...
                # Delegação de tarefas especiais primeiro
                if current_task_description.startswith("TASK_GERAR_IMAGEM:"):
                    image_prompt = current_task_description.replace("TASK_GERAR_IMAGEM:", "").strip()
                    if not image_prompt and self.completed_tasks_results and "criar descrição" in self.completed_tasks_results[-1]['task'].lower():
                        image_prompt = self.completed_tasks_results[-1]['result']
                    
                    task_result = self.image_worker.generate_image(image_prompt)
                    image_generation_attempts.append({"image_prompt_used": image_prompt, "result": task_result})
                    self.completed_tasks_results.append({"task": current_task_description, "result": task_result})
                    ...
                    for concept in validated_concepts:
                        if isinstance(concept.get("result"), str) and os.path.exists(concept.get("result")):
                            self.temp_artifacts.append({'type': 'imagem', 'artifact_path': concept.get("result"), 'prompt': concept.get('image_prompt_used')})

                else: # Tarefas gerais são roteadas
                    routing_attempts = 0
                    task_success = False
                    while routing_attempts < MAX_ROUTING_ATTEMPTS and not task_success:
                        model_to_use = self.router.route_task(current_task_description)
                        
                        task_result, new_tasks = self.worker.execute_sub_task(current_task_description, context_summary, uploaded_file_objects, model_to_use)
                        
                        suggested_model = self.router.parse_suggestion(task_result)

                        if suggested_model and suggested_model not in self.router.known_models:
                            self.router.add_model(suggested_model)
                            routing_attempts += 1
                            print_agent_message(agent_display_name, f"Tentando novamente a tarefa com o novo modelo sugerido: {suggested_model}")
                            time.sleep(1) # Pequena pausa antes de tentar novamente
                            continue
                        elif task_result is None or (isinstance(task_result, str) and task_result.startswith("Falha:")):
                            print_agent_message(agent_display_name, f"A tarefa '{current_task_description[:50]}...' falhou no modelo '{model_to_use}' sem nova sugestão. Desistindo da tarefa.")
                            self.completed_tasks_results.append({"task": current_task_description, "result": task_result})
                            break # Sai do loop de roteamento
                        else:
                            task_success = True
                            self.completed_tasks_results.append({"task": current_task_description, "result": task_result})
                            if new_tasks:
                                print_agent_message(agent_display_name, f"Worker sugeriu novas tarefas: {new_tasks}")
                                # (A lógica de confirmação e inserção de novas tarefas iria aqui)

                current_task_index += 1
            
            print_agent_message(agent_display_name, "Ciclo de tarefas concluído. Iniciando validação final.")
            final_context_for_validation = "\n".join([f"Tarefa: {r['task']}\nResultado: {str(r.get('result'))[:300]}..." for r in self.completed_tasks_results])
            
            is_valid, validation_output = self.validator.validate_and_save_final_output(initial_goal, final_context_for_validation, uploaded_file_objects, self.temp_artifacts)
            ...
            if user_choice == 'a':
                ...
                self.task_list = [f"TASK_CORRIGIR_COM_FEEDBACK: O resultado anterior não foi satisfatório (Contexto: {final_context_for_validation}). Corrija-o com base no seguinte feedback do usuário: '{feedback}'"]
                manual_retries += 1
                print_agent_message(agent_display_name, "Nova tarefa de correção definida. Iniciando novo ciclo...")
...
if __name__ == "__main__":
    ...
    print(f"--- Sistema Multiagente Gemini ({SCRIPT_VERSION} - Roteador de Modelos Dinâmico) ---")
